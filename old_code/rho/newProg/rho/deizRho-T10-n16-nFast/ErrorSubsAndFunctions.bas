...
Public Sub show_error(msg As String) ', frm_show As Form)
  Dim ch As Long
  ch = SetTopMostWindow(frmSettings.hwnd, False)
  'ch = SetTopMostWindow(frm_show.hwnd, False)
  MsgBox msg
  'ch = SetTopMostWindow(frm_show.hwnd, True)
  ch = SetTopMostWindow(frmSettings.hwnd, True)
End Sub

Public Sub GpibErr(msg As String)
  msg = msg + AddIbsta() + AddIberr() + AddIbcnt() + _
    Chr(13) + Chr(13) + "GPIB Error!"
  
  write_status msg
  MsgBox msg, vbOKOnly + vbExclamation, "GPIB Error"
  
  If frmMainForm.experiment_is_running = True Then
    Call frmMainForm.save_file
  End If
